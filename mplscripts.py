...
def extract_info_mpl(PATH: str) -> pd.DataFrame:
    ...
    # Barra de progresso para mostrar o andamento da extração
    with Progress() as progress:
        task = progress.add_task("Extraindo dados...", total=len(os.listdir(PATH)))
        ...
        for arquivo in os.listdir(PATH):
            ...
            if arquivo.endswith(".nc"):
                # print(f'Extraindo informações de: {arquivo}', end='\r')
                nome_arquivo = "/" + arquivo
                try:
                    ds = xr.open_dataset(PATH + nome_arquivo)

                    # Obtendo data e hora
                    data_str = arquivo.split("_")[2].split(".")[0]
                    ...
                    # Normalizando o AOD (Essa normalização é feita baseada na normalização do SigmaMPL)
                    ...
                    polluted_aerosol.append(
                        ds["particle_type"].data[0].tolist().count(6)
                    )
                    data_hora.append(data_str)
                    ...
                    rain_dust.append(
                        ds["particle_type"].data[0].tolist().count(3)
                    )  # Rain/Dust
                    ice_dust_ash.append(
                        ds["particle_type"].data[0].tolist().count(2)
                    )  # Ice/Dust/Ash
                    molecular.append(
                        ds["particle_type"].data[0].tolist().count(4)
                    )  # Molecular
                    water_clouds.append(
                        ds["particle_type"].data[0].tolist().count(0)
                    )  # Water Clouds
                    mixed_clouds.append(
                        ds["particle_type"].data[0].tolist().count(1)
                    )  # Mixed Clouds

                except Exception:
                    print(
                        f"Erro inesperado: Não foi possível extrair informações de {arquivo}"
                    )
                    print(str(Exception))
